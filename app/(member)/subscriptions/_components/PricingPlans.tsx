...
export const PricingPlans = ({ email, name, phoneNumber, plans }: Props) => {
  ...
  const handlePayment = (plan: GetSubscriptionPlansType) => {
    // 1. Prepare Flutterwave Config
    const config = {
      public_key: env.NEXT_PUBLIC_FW_PUBLIC_KEY,
      tx_ref: `GC-${Date.now()}`,
      amount: Number(plan.price), // Flutterwave uses actual value, not kobo
      currency: "NGN",
    ...
    handleFlutterwavePayment({
      callback: (response) => {
        // response.status can be "successful" or "completed"
        if (
          response.status === "successful" ||
          response.status === "completed"
        ) {
          // 2. Start Server Action Transition
          startTransition(async () => {
            const toastId = toast.loading(
              "Verifying payment and upgrading your account...",
            );
            ...
            const { data: result, error } = await tryCatch(
              activateSubscription({
                ...
                transactionId: String(response.transaction_id), // Passed to backend for verification
              }),
            );
          ...
          >
            ...
            <CardFooter className="mt-auto">
              <Button
                className="w-full"
                size={"md"}
                // Prevent clicking while server action is verifying
                disabled={plan.name === "Basic" || isPending}
                onClick={() => handlePayment(plan)}
              >
                {isPending ? (
                  <Loader />
