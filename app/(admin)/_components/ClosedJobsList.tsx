...
import { GetClosedJobsType } from "@/app/data/admin/job/get-closed-jobs";
import { JobBox } from "./JobBox";
import { loadMoreClosedJobs } from "@/app/data/admin/job/load-more-closed-jobs";

interface Props {
  initialJobs: GetClosedJobsType[];
  initialHasNext: boolean;
  initialTotal: number;
  query?: string;
}

export function ClosedJobsList({
  initialJobs,
  initialHasNext,
  initialTotal,
  query,
}: Props) {
  const [jobs, setJobs] = useState<GetClosedJobsType[]>(initialJobs);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const result = await loadMoreClosedJobs(nextPage, query);
