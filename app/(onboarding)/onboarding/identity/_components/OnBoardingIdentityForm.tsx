...
export function OnBoardingIdentityForm({ user }: Props) {
  ...
  function onSubmit(data: OnboardingIdentitySchemaType) {
    startTransition(async () => {
      const { data: result, error } = await tryCatch(saveIdentification(data));
    ...
    <div className="mt-8">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          />
          <div>
            <FormLabel className="mb-2.5">Proof of identity</FormLabel>
                    ...
                    >
                      ...
                      <SelectContent>
                        {identificationTypes.map((idType, index) => (
                          <SelectItem key={index} value={idType}>
                            {idType}
                          </SelectItem>
                        ))}
                      </SelectContent>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Button variant="ghost" className="w-full" size="md" asChild>
              <Link href="/">Skip</Link>
            </Button>
            <Button
              disabled={pending}
